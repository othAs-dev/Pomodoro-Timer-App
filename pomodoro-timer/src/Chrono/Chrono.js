...
const Chrono = () => {
  ...
  const [sessionFixe, setSessionFixe] = useState(1500);

  const [breakTime, setBreakTime] = useState(300);
  const [breakTimeFixed, setBreakTimeeFixed] = useState(300);
  ...
  const handleBreak = (e) => {
    ...
    if (el.classList.contains("minus")) {
      if (breakTime / 60 > 1) {
        ...
        setBreakTimeeFixed(breakTimeFixed - 60);
      }
    } else if (el.classList.contains("plus")) {
      setBreakTime(breakTime + 60);
      setBreakTimeeFixed(breakTimeFixed + 60);
